...
# \- \*\*Policy-as-code engineering\*\* — every Checkov rule maps to a specific 800-53 control via `scripts/evidence-to-oscal.py`
...
├── .github/workflows/terraform-scan.yml   # CI: Checkov + OSCAL + commit

├── infrastructure/

│   ├── providers.tf

│   └── s3-bucket.tf                        # AU-2, SC-13, CM-6

├── scripts/evidence-to-oscal.py            # SARIF → OSCAL transformer

├── oscal/

│   ├── assessment-results/                 # auto-committed per CI run

│   └── poam/medvault-poam.json             # hand-authored

├── dashboard/app.py                        # Streamlit ConMon UI

└── docs/

├── iam-policy.json                     # least-privilege evidence
...
| CM-6 | Checkov enforces configuration baseline | Checkov SARIF → Security tab |
...
\*\*`s3:\*` scoped to `medvault-\*` resources.\*\* The IAM policy uses a broad action set with a narrow resource scope. This is resource-scoped least privilege — the user can do anything to MedVault buckets but cannot enumerate other buckets in the account.



\*\*Workflow exits non-zero on findings, but branch protection is the \*real\* gate.\*\* The Checkov step fails the build on violations. That alone reports findings but doesn't enforce them — GitHub branch protection rules are what actually block merges.
...
\*\*Access log bucket uses AES256 instead of KMS.\*\* Intentional to avoid circular KMS dependency. AES256 is FIPS 140-2 validated, so SC-13 is still satisfied. Documented as POAM-003.
